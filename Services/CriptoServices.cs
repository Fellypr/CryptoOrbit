...
namespace CryptoOrbit.Services
{
    public class CriptoService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        var groqServices = scope.ServiceProvider.GetRequiredService<IGroqInterfece>();
                        foreach (var moeda in listCoins)
                        {
                            if (stoppingToken.IsCancellationRequested) break;
                            try
                            {
                                CriptoDto moedaAtualizada = await GetCryptoById(moeda,groqServices,stoppingToken);
        ...
        public async Task<CriptoDto> GetCryptoById(CriptoDto moeda,IGroqInterfece groqInterfece, CancellationToken cancellationToken)
        {
            try
            {
                var prompt = new
                {
                    ...
                    messages = new[]
                    {
                        ...
                        {
                            role = "system",
                            content = $@"# INSTRUÇÕES DO SISTEMA: Analista de Criptoativos (Output JSON)
                            ## Seu Papel: Você é um microsserviço de análise financeira estruturada. Sua única função é receber dados de uma criptomoeda, calcular métricas e retornar um objeto JSON válido que corresponda exatamente à estrutura da classe `CriptoDto`.
                            ## Restrições Estritas de Saída (Output):1. **APENAS JSON PURO:** Retorne única e exclusivamente o objeto JSON. Não inclua blocos de código (como ```json ... ```), explicações, saudações ou preâmbulos. O primeiro caractere da resposta deve ser {{ e o último deve ser }}.2. **CAMPOS OBRIGATÓRIOS NO JSON:** O JSON de saída deve conter exatamente as chaves: ""name"", ""symbol"", ""image"", ""current_price"", ""high_24h"", ""low_24h"", ""price_change_percentage_24h"", ""price_range"", ""total_volume"" e ""recommendation"".
                        ...
                        {
                            role = "user",
                            content = JsonSerializer.Serialize(moeda)
                        }
                    }
                };


                var groqResponse = await groqInterfece.InfoCryptoForCoin(prompt);
                if (string.IsNullOrWhiteSpace(groqResponse))
                {
                    throw new Exception("Erro ao obter dados da moeda");
                }

                var groqResponseLimpa = groqResponse.Trim();
                if (!groqResponseLimpa.StartsWith("{"))
                {
                    throw new InvalidOperationException($"A Groq não retornou um JSON válido. Resposta recebida: {groqResponseLimpa}");
                }

                var result = JsonSerializer.Deserialize<CriptoDto>(groqResponseLimpa);

                if (result == null)
                {
                    throw new JsonException("Não foi possível desserializar a resposta da Groq para CriptoDto.");
                }
